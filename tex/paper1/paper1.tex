...
Fourier power).  The \FPs\ used below are calculated from the mean
...
\section{Fitting models to the \PS}\label{sec:app:ind}
In order to perform the fit, the noise in the mean \FPs\ must be
estimated.  To do that, the power distributions of Section \ref{???}
are considered.  Each measurement of the Fourier power at a given
pixel is not independent of the Fourier power at another pixel.  This
is because neighbouring pixels are highly likely to be physically
...
procedure is repeated 10,000 and the mean value $\hat{\rho}$ is
...
\item Model 1 is fit to the data using the distribution widths
  $\sigma$.  Since this is much wider than the standard error in
...
\item The estimated values to the model parameters are then used as
  ...
  narrower estimated error $\sigma/\sqrt{N_{eff}}$.  This generates
