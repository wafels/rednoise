...
Figure \ref{}) must be estimated.  To do that, the power distributions
of Section \ref{sec:anal} are considered.  The Fourier power at a
given pixel is not independent of the Fourier power at another pixel.
This is because neighbouring pixels are highly likely to be physically
...
used to fit Models 1 and 2 to the data.  A Bayesian/Markov chain Monte
Carlo package (PyMC version 2.3,
...
their uncertainty.  The likelihood 

Each of the priors for the parameters in each model has a uniform
distribution, and are given in Table \ref{tab:priors}.  Each MCMC
chain takes 100,000 samples, with burn-in assumed to be at 50,000
samples.  The chain is thinned by taking every fifth element in the
chain, forming the final sample from which the parameter estimates and
their uncertainties are calculated.
...
function is constructed using. Maximum likelihood
...
Maximum likelihood estimates to the values of the parameters of
...
The 68\% credible intervals for the parameters are quoted in Table
