...
\caption{Geometric mean of the \protect\Fpa\ (black solid line) in
  ...
  detail).  The AIC describing the preference for model over the other
  is also indicated in each plot.  The reduced $chi^{2}_{r}$ statistic
  for Model 1 and Model 2 are given. The two components to Model 2,
  $P_{1}(\nu)$ and $G(\nu)$ are also shown in green; the location of
  the maximum of the $G(\nu)/P_{1}(\nu)$ ratio is indicated by the
  green dotted line.  Model fit parameter values and their 68\%
  credible intervals are quoted in Table \ref{tab:parameters}.}
\label{fig:fit171193}
\end{figure}
...
{\bf The AIC reveals which out of a set of models is preferred.  It
  ...
  Goodness-of-fit is estimated by calculating reduced-$\chi^{2}$ is
