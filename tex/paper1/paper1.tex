...
Equation \ref{eqn:sigmamean}.  The value $\sigma_{mean}$ is used as
the estimate of the error in the \FPs.  Table \ref{tab:neff} shows the
number of pixels in the region and the estimated number of effective
pixels. 
...
The \FPs\ are assumed to be normally distributed with a width given by
$\sigma_{mean}$ as calculated above.
...
defined by the data (Section \ref{sec:obs}).  A five step fitting
process is used to fit Models 1 and 2 to the data.  Maximum likelihood
estimates are found using scipy's maximum likelihood estimators
...
\cite{???} calculated using the final parameter estimates of Models 1
and 2.
