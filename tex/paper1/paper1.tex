...
The arithmetic and geometric mean power spectra were formed from all
the time-series in each region.  A third power spectrum was created by
first summing all the data in a region, applying the
normalization/apodization procedure described above, and then
calculating the Fourier power spectrum.  These three power spectra
then summarize the Fourier power in each region, for each wavebad
studied.  Figures \ref{fig:arith171} and \ref{fig:geom171} shows the
arithmetic and geometric mean power spectra for each region in AIA
171\AA.  Figures \ref{fig:arith193} and \ref{fig:geom193} show the
arithmetic and geometric mean power spectra for each region in AIA
193\AA.  Each mean power spectrum is also fit with a power spectrum of
the form
\begin{equation}
\label{eq:pwrlaw}
P(f) = Af^{-n} + C
\end{equation}
where $f$ is the frequency, $n$ is the power law index, $A$ a
normalization constant and $C$ is a background power.  Note that in
these plots, a least-squares fitting algorithm is used to fit this
model to the data.  The fit here is intended to show the approximate
power-law nature of the observed Fourier power spectrum.
...
\subsection{Initial inspection of the power spectra}\label{ssec:init}
...
in the corona is much stronger if it can be shown that a narrow
frequency-band oscillation occurs is significantly above a red-noise
