...
The estimated error in the estimated mean is
\begin{equation}
...
the actual number of pixels used in any one region.  The number of
...
the two time-series are linearly dependent on each other.  The effect
number of independent pixels in region is given by
\begin{equation}
\label(eqn:nind}
N_{ind}= 1 + \rho(N_{pixel}-1)
\end{equation}
which is used in Equation \ref{eqn:errorsigmamean} to calculate the
