                    ...
                    for i in range(0, npar):
                        ...
                        for j in range(i+1, npar):
                            ...
                            for plot_function in (1, 2):
                                ...
                                polynomial = np.polyfit(p1, p2, 1)
                                xcorr = p1_limits.value[0] + (p1_limits.value[1] - p1_limits.value[0]) * np.arange(0, 1.0 + 0.01, 0.01)
                                ycorr = np.polyval(polynomial, xcorr)
                                plt.plot(xcorr, ycorr,
                                         label='%s ~ %f %s' % (ylabel, polynomial[0], xlabel),
