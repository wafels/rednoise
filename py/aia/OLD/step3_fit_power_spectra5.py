...
#
# NOTE: USES VERSION 5 OF THE SPECTRAL MODELS
#
import pickle
import os
...
for iwave, wave in enumerate(waves):
    ...
    for iregion, region in enumerate(regions):
        ...
        for iwindow, window in enumerate(windows):
            ...
            for i in range(0, nx):
                for j in range(0, ny):
                    power_law_plus_constant_fit_storage[i, j, :] = fit_power_law_plus_constant(f, power)[:]
            ...
            # Dump the results
            filepath = os.path.join(output, ofilename + '.rnspectralmodels5.lnlike_fit_results.pkl')
