...
def write_plots(M, region_id, obstype, savefig, model, passnumber,
    ...
    if model == 'M1':
        variables = ['power_law_norm', 'power_law_index', 'background',
                 'eda_amplitude', 'eda_frequency', 'eda_index']
    if model == 'M2':
        variables = ['power_law_norm', 'power_law_index', 'background',
...
def get_variables_M1(m):
    A = [m.value.power_law_norm, m.value.power_law_index, m.value.background,
         m.value.eda_amplitude, m.value.eda_frequency, m.value.eda_index]
    return A
...
def curve_fit_M1(x, y, p0, sigma, n_attempt_limit=10):
    ...
    while (not success) and (n_attempt <= n_attempt_limit):
        try:
            answer = curve_fit(rnspectralmodels.Log_splwc_AddExpDecayAutocor_CF, x, y, p0=p_in, sigma=sigma)
            success = True
        except RuntimeError:
...
def get_spectrum_M1(x, A):
    return rnspectralmodels.Log_splwc_AddExpDecayAutocor(x, A)
    ...
    for iregion, region in enumerate(regions):
        ...
        for iwindow, window in enumerate(windows):
            ...
            for obstype in ['.logiobs']:
                ...
                if bump_loc.sum() >= 10:
                    g_estimate, _ = curve_fit(rnspectralmodels.exp_decay_autocor_CF, x[bump_loc], (pwr - M0_bf)[bump_loc])
                ...
                print "Normalized bump frequency limits : ", bump_frequency_limits
                ...
                for jjj, sigma in enumerate((sigma_of_distribution, sigma_for_mean)):
                    ...
                    pwr = pwr_ff
                    if jjj == 0:
                        if A1_estimate[4] < bump_frequency_limits[0]:
                            A1_estimate[4] = bump_frequency_limits[0]
                            print '!! Estimate fit exceeded lower limit of bump_frequency_limits. Resetting to limit'
                        if A1_estimate[4] > bump_frequency_limits[1]:
                            A1_estimate[4] = bump_frequency_limits[1]
                            print '!! Estimate fit exceeded upper limit of bump_frequency_limits. Resetting to limit'
                        pymcmodel1 = pymcmodels2.Log_splwc_AddExpDecayAutoCor(x, pwr, sigma, init=A1_estimate, normalized_frequency_limits=bump_frequency_limits)
                    else:
                        if A1[4] < bump_frequency_limits[0]:
                            A1[4] = bump_frequency_limits[0]
                            print 'First pass fit exceeded lower limit of bump_frequency_limits'
                        if A1[4] > bump_frequency_limits[1]:
                            A1[4] = bump_frequency_limits[1]
                            print 'First pass fit exceeded upper limit of bump_frequency_limits'
                        pymcmodel1 = pymcmodels2.Log_splwc_AddExpDecayAutoCor(x, pwr, sigma, init=A1, normalized_frequency_limits=bump_frequency_limits)
                ...
                normalbump_BF = np.log(rnspectralmodels.exp_decay_autocor_CF(x, A1[3], A1[4], A1[5]))
